...
A companion to `prototype.html` — covers how to use it, what interactions exist, and which design decisions are embedded in the code. Read alongside the design documents in `documents/design/`.

---

## How to open

Open `prototype.html` directly in a browser. No build step, no server required. It works from `file://`. Google Fonts loads over network; if offline, Inter falls back to system sans-serif gracefully.
