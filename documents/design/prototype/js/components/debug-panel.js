    ...
    const DEMO = {
      ...
      goWarning() {
        state.ruleEditor = { ruleId: null, draft: { ...blankRuleDraft(), name: 'High-volume test', threshold: { count: 1, window: 1, unit: 'hour' }, conditions: { sentiment: 'any', platform: 'any', authorTier: 'any', keywords: '' } }, mode: 'sidebar', step: 'form' };
      ...
      goEditFromAlert() {
        ...
        renderAll();
        setTimeout(() => openRuleEditorSidebar(a.ruleId), 100);
      },
      scenarioCalm() {
      ...
      scenarioCrisis() {
        ...
        // Inconsistent delays — feels more like real alert firing
        const delays = [0, 1400, 3100, 5800, 9200, 13500];
        delays.forEach((delay, i) => setTimeout(() => {
          const isCritical = i < 3;
          const newAlert = {
            id: 'crisis' + i, ruleId: isCritical ? 'r1' : (i === 3 ? 'r7' : 'r2'), topicId: 't1', severity: isCritical ? 'critical' : 'important',
          ...
          if (isCritical) toastCritical(newAlert);
        }, delay));
        toast('Crisis mode — multiple alerts firing');
      },
      scenarioEmpty() {
        ALERTS_DATA.length = 0;
        state.route.name = 'inbox';
